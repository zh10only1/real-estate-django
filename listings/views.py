...
def createList(request):
    ...
    if request.method == 'POST':
        ...
        form = ListingForm()
    tb = Topbar.objects.get()
    return render(request, 'listings/list_form.html', {'form' : form, 'tb':tb})


def updateList(request, list_id=None):
    interface = get_object_or_404(Listing, id=list_id)
    form = ListingForm(request.POST or None, instance=interface)
